...
func (conn *conn) readLoop() {
	...
	for !conn.closed {
		...
		if !found || chunkstream == nil {
			logger.ModulePrintf(LOG_LEVEL_TRACE, "New stream 1 csi: %d, fmt: %d\n", csi, vfmt)
		...
		if !found {
			logger.ModulePrintf(LOG_LEVEL_TRACE, "New stream 2 csi: %d, fmt: %d, header: %+v\n", csi, vfmt, header)
		}
		conn.inBytes += uint32(n)
...
func (conn *conn) received(message *Message) {
	...
	if message.Type == AGGREGATE_MESSAGE_TYPE {
		...
		for message.Buf.Len() > 0 {
			...
			if err != nil {
				logger.ModulePrintln(LOG_LEVEL_WARNING,
...
func (conn *conn) invokeAbortMessage(message *Message) {
	logger.ModulePrintln(LOG_LEVEL_TRACE,
...
func (conn *conn) invokeUserControlMessage(message *Message) {
	...
	case EVENT_PING_REQUEST:
		...
		logger.ModulePrintln(LOG_LEVEL_TRACE, "conn::invokeUserControlMessage() Ping response")
		conn.Send(respmessage)
	case EVENT_PING_RESPONSE:
		logger.ModulePrintln(LOG_LEVEL_TRACE, "conn::invokeUserControlMessage() EVENT_PING_RESPONSE")
	case EVENT_REQUEST_VERIFY:
		logger.ModulePrintln(LOG_LEVEL_TRACE, "conn::invokeUserControlMessage() EVENT_REQUEST_VERIFY")
	case EVENT_RESPOND_VERIFY:
		logger.ModulePrintln(LOG_LEVEL_TRACE, "conn::invokeUserControlMessage() EVENT_RESPOND_VERIFY")
	case EVENT_BUFFER_EMPTY:
		logger.ModulePrintln(LOG_LEVEL_TRACE, "conn::invokeUserControlMessage() EVENT_BUFFER_EMPTY")
	case EVENT_BUFFER_READY:
		logger.ModulePrintln(LOG_LEVEL_TRACE, "conn::invokeUserControlMessage() EVENT_BUFFER_READY")
	default:
